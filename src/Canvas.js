...
export default class Canvas extends React.Component {
  ...
  handleCanvasClick = (event) => {
    ...
    if (mode === "node") {
      ...
      const newNodeId = this.createNode(event, 'nodeUnf');
      if (edgeCompleting){
        this.completeEdgeWithNewNode(event, newNodeId);
      } else {
        this.createEdgeWithNewNode(event, newNodeId);
      }
    }
  }
   ...
   * @param e - the click event that the position of the node will be based on
